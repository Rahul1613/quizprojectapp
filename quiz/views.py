...
def take_exam_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        for q in questions:
            submitted_ans = request.POST.get(str(q.id))
            actual_answer = q.answer
            selected_answers[str(q.id)] = submitted_ans  # Store the selected answer
            
            if submitted_ans == actual_answer:
                marks += q.marks
    ...
    return render(request, 'quiz/take_exam.html', {'course': course, 'questions': questions})

@login_required(login_url='adminlogin')
def admin_quiz_details_view(request):
    # Get teacher statistics
    teachers = TMODEL.Teacher.objects.filter(status=True)
    teacher_stats = []
    
    for teacher in teachers:
        # Count courses and results related to this teacher
        teacher_name = f"{teacher.user.first_name} {teacher.user.last_name}"
        total_tests = models.Course.objects.all().count()  # Since Course doesn't have teacher field, showing total courses
        active_results = models.Result.objects.filter(exam__in=models.Course.objects.all()).count()
        completed_tests = active_results  # Using results count as completed tests
        
        teacher_stats.append({
            'name': teacher_name,
            'total_tests': total_tests,
            'active_tests': total_tests - completed_tests,  # Assuming remaining tests are active
    ...
    for student in students:
        ...
        if total_exams > 0:
            avg_score = sum(result.marks for result in results) / total_exams
            highest_score = max(result.marks for result in results)
            last_exam = results.order_by('-date')[0]
            last_exam_date = last_exam.date.strftime('%Y-%m-%d %H:%M')
        else:
            avg_score = 0
            highest_score = 0
            last_exam_date = "N/A"
    ...
    return render(request, 'quiz/admin_quiz_details.html', {
